...
const contractFn=`function rendererCanaryPreconditionContract_(){return{pass:true,contract:'${CONTRACT}',policy:'${POLICY}',healthMaxMs:${HEALTH_MAX_MS},requiresExplicitCanary:true,requiresRuntimeMargin:true,requiresTracePreRendererProof:true,requiresAtomicTraceReadback:true,requiresHealthConsistency:true,fullGoldenTracePassRequiredBeforeCanary:false};}`;
...
for(const token of [CONTRACT,POLICY,'rendererCanaryPreconditionsMet_','rendererGoldenTracePreRendererReady_','ATOMIC_APPEND_VERIFY_RETIRE','FL-080-CLOSED','FL-059-CLOSED',String(HEALTH_MAX_MS),'HEALTH_STATE_INCONSISTENT'])if(!s.includes(token))throw new Error('Canary precondition marker missing '+token);
...
console.log(JSON.stringify({status:'PASS',file:target,changed:s!==before,contract:CONTRACT,policy:POLICY,healthMaxMs:HEALTH_MAX_MS,explicitCanaryRequired:true,runtimeMarginRequired:true,tracePreRendererProofRequired:true,atomicTraceReadbackRequired:true,healthConsistencyRequired:true,fullGoldenTracePassRequiredBeforeCanary:false,verifiedArtifact:file},null,2));
