...
const EXACT_CONTRACT='RENDER-CAREERBREAK-EXACT3-001';
const EXACT_REGRESSION='RENDER-CAREERBREAK-EXACT3-LIVE-001';
const APPROVED=[
  'Took a planned break for caregiving and health priorities while continuing independent product and AI development and portfolio work.',
  'Built a live personal AI automation (Morning Brief) using n8n and Claude to generate a daily briefing for personal use.',
  'Developing a personal Job Copilot using Claude Code to automate job sourcing, fit scoring, tailored application preparation and workflow tracking (personal/not live).'
];
...
function rendererCareerBreakSelfTest_(){const expected=rendererCareerBreakApprovedLines_();const fixture={experiences:[{key:'independent_break',title_line:'Independent Product Building & Career Break',date_line:'Sep 2024 – Present | Bengaluru, India',bullets:expected.map(x=>({text:x,evidence_ids:[]})).concat([{text:'Morning Brief — live AI automation project summary.',evidence_ids:[]},{text:'Job Copilot — personal job workflow automation project summary.',evidence_ids:[]}])}],career_break:'Fallback narrative only'};const got=rendererCareerBreakExperienceLines_(fixture);if(got.length!==3||got.some((x,i)=>x!==expected[i]))throw new Error('${EXACT_REGRESSION} failed '+JSON.stringify(got));return{pass:true,total:3,contract:'${POLICY}',exactContract:'${EXACT_CONTRACT}',fixture:'${EXACT_REGRESSION}',experienceKey:'independent_break',semanticDuplicateFixture:true};}
function runRendererCareerBreakSelfTest(){const x=rendererCareerBreakSelfTest_();upsertWorkerState_('renderer_careerbreak_self_test',x.pass?'PASS':'FAIL',JSON.stringify(x));upsertWorkerState_('renderer_contract_version','${POLICY}','${EXACT_CONTRACT}: Independent Product Building exact canonical 3 bullets');const cur=rendererWorkerStateValue_('renderer_recurrence_gate');if(cur!=='CANARY_PASS'&&cur!=='BLOCKED_SAME_POLICY_FAILURE')upsertWorkerState_('renderer_recurrence_gate','SELF_TEST_PASS_CANARY_PENDING','Production canary must pass before backlog fan-out');return x}`;

if(s.includes('function rendererCareerBreakLines_(')){
  for(const n of ['rendererWorkerStateValue_','rendererCareerBreakLines_','rendererCareerBreakApprovedLines_','rendererCareerBreakExperience_','rendererCareerBreakExperienceLines_','rendererReplayBlocked_','rendererQuarantineBlocks_','unresolvedReleaseBlockerIds_','enforceReleaseBlockerHealth_','rendererCareerBreakSelfTest_','runRendererCareerBreakSelfTest']){
...
replaceRange('render_',old=>{
  ...
  if(current.test(old))old=old.replace(current,structural);
    ...
    old=old.replace(anchor,`/* ${EXACT_REGRESSION}: render independent_break structurally with exact canonical 3 */${structural}${anchor}`);
  }
...
for(const token of [LIVE_REGRESSION,EXACT_CONTRACT,EXACT_REGRESSION,'RENDER-CAREERBREAK-001',"careerBlock('{{CAREER_BREAK}}',ex.independent_break",'function rendererCareerBreakApprovedLines_(','function rendererCareerBreakExperienceLines_(','rendererCareerBreakExperienceLines_({experiences:[e]})','PREVENTION-RECURRENCE-001'])if(!s.includes(token))throw new Error('Live Career Break binding missing '+token);
fs.writeFileSync(file,s);
const syntax=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(syntax.status!==0)throw new Error(syntax.stderr);
console.log(JSON.stringify({status:'PASS',contract:POLICY,exactContract:EXACT_CONTRACT,regression:EXACT_REGRESSION,compatRegression:'RENDER-CAREERBREAK-001',healthReserve:'HEALTH-TAIL-RESERVE-001',criticalHealthFirst:true,structure:'independent_break title+date+exact canonical 3 bullets',semanticDuplicateFixture:true,liveCanaryRequired:true},null,2));
