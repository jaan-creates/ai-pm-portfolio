...
for(const token of ['return Number(runQ_(r)||0)','return 1;/* QUEUE-RUNQ-SUCCESS-001 */','CANARY-QUEUE-ID-BINDING-001','CANARY-TERMINAL-SUCCESS-001','CANARY-ARTIFACT-READBACK-001','CANARY-PAYLOAD-CONTRACT-001','FL-080-CLOSED','FL-059-CLOSED','function rendererCanaryPayload_()','function runRendererAuthorizedCanary()'])if(!after1.includes(token))throw new Error('Missing '+token);
...
// FL-086 behavioral producer/validator contract: evaluate the actual generated
// payload helper through the exact canary decision semantics, not marker presence.
...
const known={configuredCanary:true,payload};
if(sandbox.rendererCanaryStateDecision_(known)!==true)throw new Error('Generated canary payload rejected by known-good decision');
