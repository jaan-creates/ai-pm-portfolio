...
function SH_(){} function hm_(){} function find_(){} function obj_(){} function workNeeded_(){return true;} function hash_(){return 'h';} function enqueue_(){return 'Q-CANARY';} function upsertWorkerState_(){}
...
// Simulated prior generation: exact-ID mechanics exist, but owner-worker handoff does not.
...
function rendererFreshCanaryEnqueue_(){return{queueJobId:'Q-CANARY'};}
...
// CANARY-QUEUE-ID-BINDING-001 / CANARY-TERMINAL-SUCCESS-001 / CANARY-ARTIFACT-READBACK-001 / CANARY-EXACT-EXECUTION-001 / CANARY-PAYLOAD-CONTRACT-001
...
if(!before.includes('CANARY-EXACT-EXECUTION-001')||!before.includes('CANARY-PAYLOAD-CONTRACT-001')||before.includes('CANARY-OWNER-WORKER-001'))throw new Error('N-1 fixture invalid');
...
for(const token of ['return Number(runQ_(r)||0)','return 1;/* QUEUE-RUNQ-SUCCESS-001 */','CANARY-QUEUE-ID-BINDING-001','CANARY-TERMINAL-SUCCESS-001','CANARY-ARTIFACT-READBACK-001','CANARY-PAYLOAD-CONTRACT-001','CANARY-PATCH-UPGRADE-001','CANARY-OWNER-WORKER-001','OWNER_WORKER_TRIGGER','FL-080-CLOSED','FL-059-CLOSED','function rendererCanaryPayload_()','function runRendererAuthorizedCanary()','function runRendererAuthorizedCanaryEnqueue()'])if(!after1.includes(token))throw new Error('Missing '+token);
...
if((after1.match(/function runRendererAuthorizedCanaryEnqueue\(/g)||[]).length!==1)throw new Error('Upgrade duplicated enqueue-only canary entrypoint');
const enqueueFn=after1.slice(after1.indexOf('function runRendererAuthorizedCanaryEnqueue('),after1.indexOf('function runRendererCanaryReadback('));
if(enqueueFn.includes('rendererExactCanaryExecute_(')||enqueueFn.includes('runQ_('))throw new Error('CI canary enqueue entrypoint must not execute renderer directly');
if(!enqueueFn.includes('rendererFreshCanaryEnqueue_()')||!enqueueFn.includes('renderer_canary_pending_queue_id'))throw new Error('Owner-worker canary handoff is not durable');
...
if(payload.preconditions?.runtime!=='FL-080-CLOSED'||payload.preconditions?.trace!=='FL-059-CLOSED'||payload.executionContext!=='OWNER_WORKER_TRIGGER')throw new Error('Generated safety/context attestations incomplete');
...
console.log(JSON.stringify({status:'PASS',contract:'CANARY-EXACT-EXECUTION-001',payloadContract:'CANARY-PAYLOAD-CONTRACT-001',upgradeContract:'CANARY-PATCH-UPGRADE-001',executionContract:'CANARY-OWNER-WORKER-001',nMinusOneUpgrade:true,idempotent:true,staleCountGuard:true,successCountGuard:true,exactQueueBinding:true,knownGoodPayloadAccepted:true,missingRuntimeRejected:true,wrongTraceRejected:true,ciEnqueueOnly:true,ownerWorkerExecution:true,first:JSON.parse(first),second:JSON.parse(second)},null,2));
