...
function runP1AContractSelfTest(){const x=p1aContractSelfTest_();upsertWorkerState_('p1a_self_test',x.pass?'PASS':'FAIL',JSON.stringify(x));upsertWorkerState_('p1a_contract_version','P1-A-1','Retrieval/intake contract telemetry');return x;}

`;
if(!s.includes('function retrievalProviderRoute_('))s=s.slice(0,i)+p1a+s.slice(i);
const oldHealth="function phase1HealthTick(){const op=processOperatorCommand_();if(op)return op;";
const newHealth="function phase1HealthTick(){const op=processOperatorCommand_();if(op)return op;try{runP1AContractSelfTest();}catch(e){upsertWorkerState_('p1a_self_test','FAIL',String((e&&e.stack)||e).slice(0,1500));}";
if(s.includes(oldHealth))s=s.replace(oldHealth,newHealth);
if(!s.includes(newHealth))throw new Error('P1-A health telemetry hook missing');
for(const token of ['function retrievalProviderRoute_(','function parseJobPostingJsonLd_(','function deterministicJobParse_(','function vacancyRevalidationRequired_(','function sourcePromotionDecision_(','function p1aContractSelfTest_(','function runP1AContractSelfTest(','p1a_self_test'])if(!s.includes(token))throw new Error('P1-A contract missing '+token);
if(s!==before)fs.writeFileSync(file,s);
console.log(JSON.stringify({status:'PASS',file:target,changed:s!==before,workstream:'P1-A',telemetry:'p1a_self_test'},null,2));
