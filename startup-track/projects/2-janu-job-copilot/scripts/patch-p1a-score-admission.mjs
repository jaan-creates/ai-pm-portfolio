...
for(const token of [CONTRACT,RECOVERY,'function jdScoreAdmissionSelfTest_(','function runJdScoreAdmissionSelfTest()','function jdScoreAdmissionRecoveryTick_(','jd_score_admission_self_test','jd_score_admission_recovery_last_result'])if(!s.includes(token))throw new Error('Scoring admission contract missing '+token);
...
console.log(JSON.stringify({status:'PASS',file:target,changed:s!==before,contract:CONTRACT,recovery:RECOVERY,newDecisionScoreAdmission:true,resumeStillApplyGated:true,recoveryScope:'golden_trace_application_id',maxRecoveryAppsPerHealthTick:1,circuitBypass:false,syntax:true},null,2));
