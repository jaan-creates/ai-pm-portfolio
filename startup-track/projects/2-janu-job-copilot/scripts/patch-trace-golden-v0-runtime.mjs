...
function runTest(script,label){const r=spawnSync(process.execPath,[path.resolve(dir,script),path.resolve(dir,'..')],{encoding:'utf8'});if(r.status!==0)throw new Error(r.stderr||r.stdout||label+' failed');return r.stdout;}
...
replaceFunction('traceNextAppId_',`function traceNextAppId_(){const sh=SH_(JC.S.A),m=hm_(sh),day=Utilities.formatDate(new Date(),Session.getScriptTimeZone(),'yyyy-MM-dd');if(!m['Application ID'])throw new Error('DETERMINISTIC:TRACE_APPLICATION_ID_HEADER_MISSING');const values=sh.getLastRow()>1?sh.getRange(2,m['Application ID'],sh.getLastRow()-1,1).getDisplayValues().flat():[];return traceNextAppIdFromValues_(day,values);}`);
replaceFunction('traceFindAppByUrl_',`function traceFindAppByUrl_(url){const d=traceFindAppByUrlDetail_(url);return d.found?d.applicationId:'';}`);
replaceFunction('traceCreateGoldenApplication_',`function traceCreateGoldenApplication_(req){const detail=traceFindAppByUrlDetail_(req.url);if(detail.found){upsertWorkerState_('golden_trace_duplicate_evidence',JSON.stringify(detail).slice(0,1500),'Duplicate requires exact non-empty canonical URL equality');return{status:'DUPLICATE',applicationId:detail.applicationId,match:detail};}const sh=SH_(JC.S.A),id=traceNextAppId_();if(find_(JC.S.A,id))throw new Error('DETERMINISTIC:TRACE_GOLDEN_ID_COLLISION:'+id);const h=sh.getRange(1,1,1,sh.getLastColumn()).getDisplayValues()[0],o={'Application ID':id,'Date Added':new Date(),'Company':req.company||'','Role':req.role||'','Job URL':req.url||'','Source':'System Source / TRACE-GOLDEN-01','Posting Date':req.postingDate||'Fresh public source','Location / Work Mode':req.location||'','Decision':'New','Status':'Verifying JD','Compensation':req.compensation||'Not published — verify','Resume Variant':req.resumeVariant||'Senior PM — JD tailored','Next Action':'System-owned: retrieve and verify canonical JD.','Link Health':'Official/public source verified for TRACE-GOLDEN-01','Canonical Apply URL':req.url||'','JD Snapshot Status':'Not Started','Submission Ready?':'No','Your Required Action':'No action','Preparation Completeness':'10%','Last Operator Update':new Date(),'JD Completeness %':0,'Source Reliability':'Official ATS','Resume Review Decision':'Not Ready','ATS QA Status':'Not Started','State Consistency':'OK'};sh.appendRow(h.map(k=>o[k]===undefined?'':o[k]));if(!find_(JC.S.A,id))throw new Error('DETERMINISTIC:TRACE_GOLDEN_APPLICATION_WRITE_READBACK_FAILED:'+id);const input=hash_(id+'|'+traceCanonicalUrl_(req.url)+'|TRACE-GOLDEN-01');const q=enqueue_(id,'JD_RETRIEVE',{source:'TRACE-GOLDEN-01',traceId:req.traceId||'TRACE-GOLDEN-01'},input);if(!q)throw new Error('DETERMINISTIC:TRACE_GOLDEN_JD_ENQUEUE_FAILED');upsertWorkerState_('golden_trace_application_id',id,'TRACE-GOLDEN-01 same-fixture lock');upsertWorkerState_('golden_trace_status','INTAKE_ACCEPTED',JSON.stringify({traceId:req.traceId||'TRACE-GOLDEN-01',applicationId:id,queueJobId:q}));return{status:'CREATED',applicationId:id,queueJobId:q};}`);
replaceFunction('traceGoldenSelfTest_',`function traceGoldenSelfTest_(){const c=[];c.push(traceCanonicalUrl_('https://jobs.ashbyhq.com/x/abc/?utm_source=z')==='https://jobs.ashbyhq.com/x/abc');c.push(traceUrlIdentityMatch_('https://jobs.ashbyhq.com/tekion/e0956a72-ce85-4e10-a34d-f5c4d630d8e0','https://jobs.ashbyhq.com/tekion/e0956a72-ce85-4e10-a34d-f5c4d630d8e0?utm_source=x')===true);c.push(traceUrlIdentityMatch_('https://jobs.ashbyhq.com/tekion/e0956a72-ce85-4e10-a34d-f5c4d630d8e0','https://job-boards.greenhouse.io/easyship/jobs/4706111006?gh_jid=4706111006')===false);c.push(traceUrlIdentityMatch_('','')===false);c.push(traceNextAppIdFromValues_('2026-08-23',['2026-08-23-001'])==='2026-08-23-002');c.push(traceDecisionPolicy_(80)==='Apply');c.push(traceDecisionPolicy_(79)==='Hold');c.push(traceDecisionPolicy_(69)==='Skip');c.push(traceDurationMs_('2026-08-23T00:00:00Z','2026-08-23T00:00:01Z')===1000);if(c.some(x=>!x))throw new Error('TRACE-GOLDEN-01 self-test failed '+JSON.stringify(c));return{pass:true,total:c.length,contract:'TRACE-GOLDEN-V0-2'};}`);
...
runTest('test-owned-edit-intake-admission.mjs','owned edit wakeup regression');
runTest('test-owner-canary-fresh-replacement.mjs','fresh canary replacement regression');
runPatch('patch-owned-edit-intake-admission.mjs','owned edit intake/apply admission');
...
for(const token of ['TRACE-GOLDEN-V0-2','P1-A-E2E-CONTINUATION-3','P1-A-E2E-CONTINUATION-4','E2E-CURSOR-FAIRNESS-001','E2E-STRANDED-JD-001','QUEUE-NOJOB-QUARANTINE-001','ATOMIC_APPEND_VERIFY_RETIRE','HEALTH-RUNTIME-RESERVE-001','REGRESSION-HEALTH-FINAL-LOCK-001','CANARY-PRECONDITION-001','OWNED-EDIT-WAKEUP-001','CONTROL-PLANE-CANARY-FRESH-001'])if(!s.includes(token))throw new Error('Integrated release safety contract missing '+token);
fs.writeFileSync(file,s);
