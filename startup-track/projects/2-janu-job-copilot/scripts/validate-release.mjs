...
const expectedVersion=process.env.EXPECTED_VERSION||'1.3.6';
const expectedSuite=process.env.EXPECTED_SUITE||'p0-regression-v17';
let files=[];
...
for(const token of ['1.3.5','p0-regression-v16','1.3.4','p0-regression-v15','1.3.3','p0-regression-v14','1.3.2','p0-regression-v13','1.3.1','p0-regression-v12','1.3.0','p0-regression-v11','1.2.9','p0-regression-v10'])if(text.includes(token))fail(`stale release identity literal present: ${token}`);
...
for(const required of ['PACK-SAN-001','QA-REPAIR-001','BOOTSTRAP-003','BOOTSTRAP-004','BOOTSTRAP-005','BOOTSTRAP-006','CONTROL-001','CONTROL-002','RELEASE-001'])if(!text.includes(required))fail(`${required} missing`);
...
if(!text.includes("'p0_closure_phase','REGRESSION','Bounded PREPARE complete; strict gates next'"))fail('FL-033 PREPARE does not advance directly to REGRESSION');
if(!text.includes("p0PrepareAction_()==='DEFER_TO_STRICT_GATES'"))fail('BOOTSTRAP-006 bounded PREPARE regression missing');

const sha=crypto.createHash('sha256').update(text).digest('hex');
console.log(JSON.stringify({status:process.exitCode?'FAIL':'PASS',expectedVersion,expectedSuite,files:files.map(f=>path.basename(f)),functions:fnNames.length,sha,fixes:['FL-031','CONTROL-002','FL-033']},null,2));
